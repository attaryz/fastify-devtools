...
class MockSchema {
  static Types = { Mixed: class Mixed {}, ObjectId: class ObjectId { constructor(_id?: string) {} } }
...
const mockModel = {
  find: (_f?: any) => ({
    sort: (_s?: any) => ({
      limit: (_l?: number) => ({ lean: () => Promise.resolve([]), cursor: () => { const { EventEmitter } = require('events'); const ee = new EventEmitter(); process.nextTick(() => ee.emit('end')); return ee; } })
    })
  }),
...
const mockMongoose = {
  ...
  Schema: MockSchema,
  Types: { Mixed: class Mixed {}, ObjectId: class ObjectId { constructor(_id?: string) {} } }
}
...
test('storage filters and branches', async (t) => {
  ...
  try {
    ...
    for (const band of bands) {
      const res = await fastify.inject({ method: 'GET', url: `/__devtools/store/requests?status=${band}&q=test&from=2024-01-01&to=2024-12-31` })
      t.equal(res.statusCode, 200)
    }

    // Exercise beforeId path (ObjectId)
    const res2 = await fastify.inject({ method: 'GET', url: `/__devtools/store/requests?limit=10&beforeId=65ff2b5ec0ffee0000000001` })
    t.equal(res2.statusCode, 200)

    // Exercise JSON export error catch by making find throw
    const originalFind = (mockModel as any).find
    ;(mockModel as any).find = () => { throw new Error('boom') }
