...
import type { DevtoolsOptions, DevtoolsEntry, WebSocketMessage } from "./types"
...
const devtoolsPlugin: FastifyPluginAsync<DevtoolsOptions> = async (
  ...
  if (trackRedisCache && detectRedis(fastify)) {
    ...
    fastify.decorate('getRedisClient', function(this: FastifyInstance, requestId?: string) {
      const client = this.redis || (this as any).ioredis
  ...
  fastify.addHook("onResponse", async (request: FastifyRequest, reply: FastifyReply) => {
    onResponseHook(
      ...
      pending,
      push,
      (entry) => persistEntry(fastify, entry, persistTtlDays)
    )
  })
