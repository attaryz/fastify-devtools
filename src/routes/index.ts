...
export function registerRoutes(
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        reply.code(404).send({ error: "Not Found", message: "Entry not found" })
      }
    }
  )
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      if (!(await requireAuth(request, reply))) return
      reply.send(wsMessages.slice(-100))
    }
  )
  ...
  // Redis status
  fastify.get(
    `${basePath}/redis/status`,
    async (request: FastifyRequest, reply: FastifyReply) => {
      if (!(await requireAuth(request, reply))) return
      const redisDetected = detectRedis()
      let redisInfo: any = { detected: redisDetected }
      
      if (redisDetected) {
        try {
          const client = fastify.redis || (fastify as any).ioredis
          if (client && client.status) {
            redisInfo.status = client.status
          }
